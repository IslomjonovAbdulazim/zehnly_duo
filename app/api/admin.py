...
async def upload_word_image(
    ...
    image_url = await storage_service.save_image(file, word_id)
...
async def _save_audio_from_data(audio_data: bytes, filename: str, entity_id: int) -> str:
    """Helper function to save audio data as a file and upload to storage"""
    import tempfile
    import os
    
    logger = logging.getLogger(__name__)
    logger.info(f"📄 Saving audio data to temp file: {filename}")
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.m4a') as temp_file:
        ...
        temp_file.flush()
        logger.info(f"💾 Written to temp file: {temp_file.name}")
        
        # Create an UploadFile-like object for the storage service
        class AudioFile:
